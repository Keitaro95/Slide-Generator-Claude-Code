...
`forclaude/subagents/slide-design-ocr.md` をロード。
既存PPTXのレイアウト・デザイン構造を読み取り、Content.theme に反映する。

**Step 2 — slide-ocr**
`forclaude/subagents/slide-ocr.md` をロード。
...
`forclaude/subagents/design-conductor.md` をロード。
Content.theme のカラー・フォント・レイアウト方針を決定する。

**Step 4 — text-conductor**
`forclaude/subagents/text-conductor.md` をロード。
...
`forclaude/subagents/subagent-converter.md` をロード。
Content.sections → slideData 配列に変換する。
コードブロックは画像プレースホルダー（imgサンプル）に変換する。

**Step 6 — subagent-slidecode**
`forclaude/subagents/subagent-slidecode.md` をロード。
...
`forclaude/subagents/subagent-scoring.md` をロード。
...
`forclaude/subagents/subagent-refactoring.md` をロード。
